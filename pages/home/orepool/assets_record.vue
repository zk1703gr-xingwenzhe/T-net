	...
	<view class="main">
		<view class="title_top">
			...
			<view class="nav">
				...
				<view class="">{{lang.financialArecords}}</view>
				<view class="">
			...
			<view class="conter" v-if="data!=''">
				<view class="conter_list" v-for="(todo,index) in data" :key="index">
					<view class="conter_left">+{{results(todo.total_assets)}}ST</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.getUserManageMoneyAssetsLog()
		},
		onReachBottom(){
			this.loading()
		},
		methods: {
			...
			getUserManageMoneyAssetsLog:function(){
				uni.showLoading({title: this.$store.state.lang.lang.propertyLoading});
				this.$userRequest
				.get('UserManageMoneyAssetsLog',{page:this.page,offset:this.offset})
				.then(res => {
					uni.hideLoading();
					if(res.data.code==0){
